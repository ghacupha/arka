...
public class MoreCollectors {
    /**
     * Collector which traverses a stream and returns either a single element (if there was only one element) or empty (if there were 0 or more than 1 elements).  It traverses the entire stream, even
     * if two elements have been encountered and the empty return value is now certain.
     * <p>
    ...
    /**
     * Same behavior as {@link #singleOrEmpty}, except that it returns early if it is possible to do so.  Unfortunately, it is not possible to implement early-return behavior using the Collector
     * interface, so MoreCollectors takes the stream as an argument.
     * <p>
     ...
     * </pre>
     *
     * @param codePoints a {@link java.util.stream.IntStream} object.
     * @return a {@link java.lang.String} object.
     */
    public static String codePointsToString(IntStream codePoints) {
        return codePoints.collect(StringBuilder::new, StringBuilder::appendCodePoint, StringBuilder::append)
                         .toString();
    }
}
