...
public class StackDumper {
     ...
     *
     * @param message a {@link java.lang.String} object.
     * @param stackLimit a int.
     * @param classPrefixesToExclude a {@link java.lang.String} object.
     */
    ...
    /**
     * Returns a PrintStream which will redirect all of its output to the source PrintStream. If the trigger string is passed through the wrapped PrintStream, then it will dump the stack trace of the
     * call that printed the trigger.
     *
    ...
    public static List<StackTraceElement> captureStackBelow(Class<?>... clazzes) {
        ...
        Predicate<StackTraceElement> isSkipped = element -> toIgnore.stream()
                                                                    .anyMatch(clazz -> {
                                                                        String name = element.getClassName();
                                                                        return name.equals(clazz.getName()) || name.startsWith(clazz.getName() + "$$Lambda");
                                                                    });

        List<StackTraceElement> rawStack = Arrays.asList(Thread.currentThread()
                                                               .getStackTrace());
